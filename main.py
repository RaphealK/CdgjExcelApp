...
C = {
    "primary": get_color_from_hex("#3F51B5"),
    "accent": get_color_from_hex("#448AFF"),
...
# --- Custom Widget Base Classes for consistent styling ---
...
def show_popup_global(title, message):
    ...
    msg_label = ThemedLabel(
        text=message, size_hint_y=None, halign='left', valign='top', color=C["text_secondary"]
    )
    ...
    popup = Popup(
        title=title, title_font='AppFont', title_color=C["primary"], content=content, 
        size_hint=(0.9, 0.6), separator_color=C["primary"],
...
class StartupScreen(Screen):
    ...
    def handle_selection(self, selection):
        if selection:
            self.excel_path_input.text = selection[0]
            self.add_log(f"已选择文件: {selection[0]}")
        else:
    ...
    def start_app(self, instance):
        ...
        try:
            ...
            self.add_log(f"文件加载成功! 记录总数: {len(app.asset_db.df)}")
            self.manager.get_screen('main').reset_session()
            self.manager.current = 'main'
        except Exception as e:
            self.show_popup("启动错误", f"加载Excel时发生错误: {e}\n{traceback.format_exc()}")
...
class MainScreen(Screen):
    ...
    def update_ui_for_state(self):
        ...
        self.stats_label = ThemedLabel(
            text=f'本轮已录入: {self.current_count} 条', font_size='14sp', color=C["text_secondary"]
        )
        ...
        if self.state == 'INPUT':
            ...
            self.build_verification_ui()
        elif self.state == 'FORM':
            self.build_form_ui()
            
        footer_layout = BoxLayout(size_hint_y=None, height='44dp', spacing='10dp')
        back_to_start_btn = ThemedButton(text="返回首页")
        back_to_start_btn.bind(on_press=self.back_to_start)
        footer_layout.add_widget(back_to_start_btn)

        export_btn = ThemedButton(text="导出数据")
    ...
    def build_input_ui(self):
        ...
        card.add_widget(self.asset_input)
        submit_btn = ThemedButton(text='提交查询', size_hint_y=None, height='44dp')
    ...
    def build_verification_ui(self):
        ...
        # 为了更好地处理长文本，我们为Label设置text_size
        ...
        correct_btn = ThemedButton(text='正确, 下一步')
        correct_btn.bind(on_press=lambda x: self.change_state('FORM'))
        incorrect_btn = Button(text='错误, 返回', font_name='AppFont', background_color=C["text_secondary"], background_normal='')
        incorrect_btn.bind(on_press=lambda x: self.change_state('INPUT'))
    ...
    def build_form_ui(self):
        ...
        self.inputs['meter_type'] = Spinner(
            ...
            font_name='AppFont', background_color=C["accent"]
        )
        ...
        self.inputs['box_type'] = Spinner(
            ...
            size_hint_y=None, height='44dp', font_name='AppFont', background_color=C["accent"]
        )
        ...
        btn_layout = BoxLayout(size_hint_y=None, height='44dp', spacing='10dp')
        submit_btn = ThemedButton(text='提交保存')
        submit_btn.bind(on_press=self.save_data)
        btn_layout.add_widget(submit_btn)
        back_btn = Button(
            text='返回上一步', font_name='AppFont', background_color=C["text_secondary"], background_normal=''
        )
        back_btn.bind(on_press=lambda x: self.change_state('VERIFY'))
    ...
    def check_asset(self, instance):
        ...
        if not last_6_digits:
            self.show_popup("输入错误", "资产号不能为空。")
            return
        matches = App.get_running_app().asset_db.get_info_by_last_6_digits(last_6_digits)
        self.asset_input.text = ""
        if not matches:
            self.show_popup("未找到记录", f"数据库中不存在以 '{last_6_digits}' 结尾的资产号。")
        elif len(matches) == 1:
            self.user_info = matches[0]
            self.change_state('VERIFY')
        else:
    ...
    def show_duplicate_selection_popup(self, matches):
        ...
        for record in matches:
            ...
            btn = ThemedButton(
                text=display_text, size_hint_y=None, height='60dp', 
                text_size=(Window.width * 0.7, None), halign='center'
            )
        ...
        self.popup = Popup(
            title="选择重复数据", title_font='AppFont', title_color=C["primary"],
            content=scroll, size_hint=(0.9, 0.8), background='', background_color=C["card"]
        )
        self.popup.open()
      
    def select_duplicate(self, record, instance):
        self.popup.dismiss()
        self.user_info = record
        self.change_state('VERIFY')
    ...
    def save_data(self, instance):
        ...
        try:
            ...
            self.show_popup("未知错误", f"保存数据时发生错误: {str(e)}")

    def export_data(self, instance):
        if self.output_path and os.path.exists(self.output_path):
            self.show_popup("导出成功", f"数据已保存到设备“下载”文件夹:\n{self.output_path}")
        else:
            self.show_popup("无数据", "尚未录入任何数据，无文件可导出。")
