...
COLOR_BACKGROUND = get_color_from_hex('#F5F5F5')
...
class StartupScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        path_layout = BoxLayout(size_hint_y=None, height=dp(48))
        self.excel_path_input = TextInput(
            hint_text="请点击“浏览”选择台账文件",
            text="",
            readonly=True,
            background_color=COLOR_WHITE,
    ...
    def copy_and_process_uri(self, uri):
        try:
            ...
            file_name = f"temp_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
            if cursor:
            ...
            self.add_log(f"开始复制文件到内部缓存: {local_path}")
    ...
    def start_app(self, instance):
        ...
        if not excel_path:
            self.show_popup("提示", "请先选择一个Excel台账文件")
            return
        ...
        try:
            ...
            self.add_log(f"文件加载成功! 记录总数: {row_count}")
...
class MainScreen(Screen):
    ...
    def create_input_ui(self):
        ...
        self.layout.add_widget(Label(text='输入原表资产号后6位:', size_hint_y=None, height=dp(30), color=COLOR_TEXT, font_size=dp(18)))
        self.asset_input = TextInput(multiline=False, size_hint_y=None, height=dp(48), font_size=dp(18), halign='center')
        self.asset_input.focus = True
        self.layout.add_widget(self.asset_input)

        submit_btn = Button(text='提交查询', size_hint_y=None, height=dp(48), background_color=COLOR_PRIMARY, color=COLOR_WHITE)
        submit_btn.bind(on_press=self.check_asset)
        self.layout.add_widget(submit_btn)

        self.stats_layout = BoxLayout(size_hint_y=None, height=dp(48))
        self.stats_label = Label(text=f'本轮已录入: {self.current_count}条', color=COLOR_TEXT)
        ...
        back_btn = Button(text='返回首页', size_hint_y=None, height=dp(48), background_color=COLOR_DANGER, color=COLOR_WHITE)
        back_btn.bind(on_press=self.back_to_start)
        self.layout.add_widget(back_btn)

        self.layout.add_widget(Label(
            text="By 延寿供电所 K", font_size=dp(12), size_hint_y=None, height=dp(30), color=(0.5, 0.5, 0.5, 1)
        ))
    ...
    def show_verification_screen(self, user_info):
        ...
        self.layout.add_widget(Label(text="请核对以下信息：", size_hint_y=None, height=dp(40), font_size=dp(20), color=COLOR_TEXT))
        self.layout.add_widget(Label(text=user_text, size_hint_y=0.3, font_size=dp(18), color=COLOR_PRIMARY))
        
        self.layout.add_widget(BoxLayout(size_hint_y=0.4))

        btn_layout = BoxLayout(size_hint_y=None, height=dp(50), spacing=dp(10))
        correct_btn = Button(text='正确，下一步', background_color=COLOR_SUCCESS, color=COLOR_WHITE)
        correct_btn.bind(on_press=partial(self.show_detail_input, user_info))
        incorrect_btn = Button(text='错误，返回', background_color=COLOR_DANGER, color=COLOR_WHITE)
    ...
    def show_detail_input(self, user_info, instance):
        ...
        header_text = f"正在为 [b]{user_info.get('用户名', '')}[/b] 录入信息"
        self.layout.add_widget(Label(text=header_text, markup=True, size_hint_y=None, height=dp(40), font_size=dp(18), color=COLOR_TEXT))
        ...
        fields = [
            ...
            ('新资产号', 'new_asset', ''), ('铅封号', 'seal_number', ''),
            ('材料使用', 'material_usage', ''), ('备注', 'remark', '')
        ]

        for label_text, name, default in fields:
            form_layout.add_widget(Label(text=label_text, color=COLOR_TEXT))
            inp = TextInput(text=default, multiline=False, size_hint_y=None, height=dp(44))
            self.inputs[name] = inp
            form_layout.add_widget(inp)

        form_layout.add_widget(Label(text='表计类型', color=COLOR_TEXT))
        self.inputs['meter_type'] = Spinner(text='单相表', values=('单相表', '三相表'), size_hint_y=None, height=dp(44), background_color=COLOR_PRIMARY)
        form_layout.add_widget(self.inputs['meter_type'])

        form_layout.add_widget(Label(text='表箱类型', color=COLOR_TEXT))
        self.inputs['box_type'] = Spinner(
            text='利旧未换', values=('利旧未换', '单位', '双位', '双位单装'),
            size_hint_y=None, height=dp(44), background_color=COLOR_PRIMARY)
        ...
        submit_btn = Button(text='提交保存', background_color=COLOR_SUCCESS, color=COLOR_WHITE)
        submit_btn.bind(on_press=self.save_data)
        btn_layout.add_widget(submit_btn)

        back_btn = Button(text='返回上一步', background_color=COLOR_DANGER, color=COLOR_WHITE)
    ...
    def get_output_path(self):
        ...
        if self.output_path is None or os.path.dirname(self.output_path) != output_dir:
            # ==================== 功能优化：修改文件名日期格式 ====================
            now_str = datetime.now().strftime("%Y年%m月%d日%H时%M分")
            # ====================================================================
    ...
    def export_data(self, instance):
        ...
        if self.output_path and os.path.exists(self.output_path):
            ...
            self.show_popup("提示", "尚未录入任何数据，无文件可导出")
    # =======================================================================
    
    # handle_export_selection 方法已被删除
